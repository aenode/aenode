...
export class ScopeService {
  constructor(
    @InjectDelegate(D.ScopeModelName)
    protected readonly delegate: D.ScopeDelegate,
  ) {}

  async findMany(query: D.ScopeQueryDto) {
    const { orderBy, orderDir, skip, take, search } = query;
    const where = D.toScopeWhereObject(search);
  ...
  create(data: D.ScopeCreateDto) {
    return this.delegate.create({ data });
  }

  updateOneById(id: number, data: D.ScopeUpdateDto) {
